...
formula = (
           "cost_per_mile ~   month + day_of_week + C(Year) + "
           "booking_days_log + booking_days_log*C(fare_type, "
...
def _bootstrap_resample(df, formula, original_model, n = 10000, booking_days_col = 'booking_days_log'):
    '''
    ...
    exog_cont = ['cost_per_mile']
    exog_fact = ['month', 'day_of_week', 'Year', 'city_pair_code', 'self_booking_indicator']
    ...
    save(param_stats=param_stats,y_pred_stats=y_pred_stats,y_preds=y_preds)
...
#Write results
def save(param_stats,y_pred_stats,y_preds):
    with open('models/segment_log_param_stats.pkl', 'wb') as f:
        pickle.dump(param_stats, f)
        
    with open(f'models/segment_log_y_pred_stats.pkl', 'wb') as f:
        pickle.dump(y_pred_stats, f)
        
    with open(f'models/segment_log_y_preds.pkl', 'wb') as f:
        pickle.dump(y_preds, f)



#this takes a long time. Could possibly refactor to improve performance
param_stats, y_pred_stats, y_preds = _bootstrap_resample(df, formula, result, n = 10000)
